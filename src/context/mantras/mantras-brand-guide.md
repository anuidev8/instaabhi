# School of Mantras — Style Reference
### v7 — Engine-Aligned (April 2026)

> This file is a **reference only**. All production rules live in `/thumbnail-engine/config/core-rules.json`.
> Do NOT use this file to override engine rules.

---

## Brand Identity

**Channel:** School of Mantras
**Niche:** Hindu devotional mantras, meditation, spiritual protection, abundance, healing
**#1 Proven Performer:** Ganesha + 2026 + Success + "Remove All Obstacles" (4.7K views)

---

## Core Visual Rules (from core-rules.json)

| Rule | Value |
|------|-------|
| Canvas | 1280×720, 16:9 |
| Style | Cinematic Divine Aura — photorealistic cinematic devotional |
| Layout | Deity LEFT 40-45%, Text RIGHT 55-60% |
| Character | Extreme close-up, face fills 75-80%+ of left zone |
| Text | 3-5 words, 2-line hierarchy |
| Background | Deep dark gradient, ultra-dark text area |
| Visual trigger | ONE subtle trigger only |

---

## Deity Aura Colors (from deities.json)

| Deity | Aura Color |
|-------|-----------|
| Shiva | Electric blue #0066FF |
| Hanuman | Fiery orange #FF4500 |
| Krishna | Teal-blue #00BFFF |
| Lakshmi | Gold #FFD700 |
| Durga | Deep violet #6A0DAD |
| Ganesha | Dark orange #FF8C00 |
| Kali | Deep crimson #8B0000 |
| Saraswati | Warm ivory #FFFACD |
| Ram | Gold #FFD700 |
| Medicine Buddha | Deep blue #1E40AF |

---

## Line 2 Colors by Intent (from intents.json)

| Intent | Line 2 Color |
|--------|-------------|
| Abundance | Gold #FFD700 |
| Protection | Blue #4D9FFF |
| Healing | Red #FF3B3B |
| Love | Pink #FF69B4 |
| Power | Orange #FF6600 |
| Peace | Soft blue #B0D4F1 |
| Knowledge | Gold #FFD700 |
| Transformation | Red #FF3B3B |

---

## Proven Title Formulas

**Formula 1 (TOP PERFORMER):**
`[Deity] Mantra for [Year] [Benefit] | [Action] [Obstacle] & [Promise]`

**Formula 2 (PROTECTION):**
`[Deity] Mantra for [Intent] [Year] | [Shield/Protect] Yourself from [Negative Thing]`

**Formula 3 (CHALLENGE):**
`[Deity] Mantra for [Benefit] & [Benefit] [Listen Daily for X Days]`

**Formula 4 (HEALING):**
`[Deity] Mantra — [Type] for [Benefit], [Benefit] & [Benefit]`

---

## What to Avoid

- No generic text (DIVINE LOVE, SACRED HEALING)
- No full body shots — extreme close-up only
- No always-gold — use deity-specific aura colors
- No glow/particles behind text
- No same-weight text lines
- No both-lines-white
- No center composition
- No cartoon/illustration/painting
