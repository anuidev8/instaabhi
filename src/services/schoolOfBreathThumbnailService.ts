    ...
    prompts.map(async (prompt) => {
      ...
      const compositionLockText =
        plan.prompt.schoolOfBreath?.mode === 'without_character'
          ? 'Attached image = LAYOUT/COMPOSITION REFERENCE (not face identity). Match tag colors and strip order. Without character in this render: keep lower-center open cosmic space for a future Abhi composite; put the proof/support graphic bottom-left, modest size (not a huge centered circle). Red CTA mid-right. Add corner chakra symbols in both top corners; symbol colors and glyph style must follow the current topic/reference image (not fixed colors). Do NOT draw any center placeholder box/panel/column/mask. If the prompt says WITH_CHARACTER, ignore this reserve and follow the prompt’s subject rules. All wording from the prompt below.'
          : 'Attached image = LAYOUT/COMPOSITION REFERENCE only (not face identity). Match: white-on-dark top strip, **black-on-yellow-gold gradient** main hook bar (thin black outline, premium gold — not flat neon) behind the subject’s shoulders, white-on-red CTA on the right torso, circular proof graphic bottom-left, curved arrow from the hook bar toward that circle, and corner chakra symbols in both top corners using topic/reference-appropriate colors and glyph style. Reproduce tag colors (white / gold hook / black text / red CTA). All wording must come from the prompt below — do not copy letters from the reference if they differ.';
